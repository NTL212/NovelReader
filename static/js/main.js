...
function setupNavigation() {
    ...
    // Sort & Jump
    const sortBtn = document.getElementById('reverse-sort');
    if (sortBtn) sortBtn.onclick = toggleSort;
    
    const jumpInput = document.getElementById('jump-to-chapter');
    if (jumpInput) {
...
function setupInfiniteScroll() {
    ...
    const observer = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting && chapterView && !chapterView.classList.contains('hidden')) {
            loadMoreChapters();
        }
    }, { threshold: 0.1 });
...
function switchView(view) {
    ...
    if (progressContainer) progressContainer.classList.add('hidden'); // Default hide progress
    ...
    if (view === 'reader') {
        ...
        if (progressContainer) progressContainer.classList.remove('hidden'); // Show progress in reader
